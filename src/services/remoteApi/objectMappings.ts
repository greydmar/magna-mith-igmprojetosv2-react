...
export const mapFromServerModelIndice = (item: ServerModels.Indice): Indice => {
  return {
    id: item.id ?? "falha",
...
export const mapFromServerModelIndices = (response: ServerModels.Indice[]): Indice[] => {
  const lResult : Indice[] = response.map( mapFromServerModelIndice) ?? [];
  return lResult;
};

export const mapFromServerModelQuestao = (item: ServerModels.Questao): Questao => {
  return {
    id: item.id ?? "falha",
...
export const mapToServerModelReqQuestao = (idQuestionario:string, item: Questao): ServerModels.ReqRespostaQuestao => {
  return {
...
export const mapToServerModelReqResposta = (form: Questionario): ServerModels.ReqRespostaQuestionario => {
...
export const mapToServerModelReqRespostaParcial = (form: Questionario, 
  lista: Questao[]): ServerModels.ReqRespostaQuestionario => {
...
export const mapFromServerModelQuestionario = (response: ServerModels.Questionario): Questionario => {
  ...
  const lQuestoes : Questao[] = response.items?.map( mapFromServerModelQuestao) ?? [];

  return {id: id, ratingCalculado: fatorCalculado, codigoIndice: codigoIndice, versao:versao, questoes: lQuestoes};
};
}